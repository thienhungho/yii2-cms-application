...
class PostController extends Controller
{
    ...
    public function actionTerm($post_type = 'post', $term_type, $slug)
    {
        ...
        if (!empty($term)) {
            $seo = Seo::find()
                ...
                ->one();
            $seo->generateSeoMetaTag(['og_locale' => $term->language, 'og_type' => $post_type]);

            $dataProvider = new ActiveDataProvider([
                'query' => Post::find()->where(['in', 'id', get_all_obj_id_in_term($term_type, $term->id, $post_type)]),
                'pagination' => [
                    'pageSize' => 20,
                ],
            ]);
    ...
    public function actionView($type = 'post', $slug)
    {
        ...
        if (!empty($model)) {
            $seo = Seo::find()->where(['obj_id' => $model->primaryKey])->andWhere(['obj_type' => $model->post_type])->one();
            $seo->generateSeoMetaTag(['og_locale' => $model->language, 'og_type' => $model->post_type]);
            $comment = new Comment();
